...
type Context={user:User|null;googleEnabled:boolean|null;loading:boolean;error:string;watchlist:string[];screens:SavedScreen[];alerts:OwnerAlert[];states:AlertState[];signIn:()=>Promise<void>;sendMagicLink:(email:string)=>Promise<boolean>;signOut:()=>Promise<void>;toggleWatch:(ticker:string)=>Promise<void>;saveScreen:(name:string,definition:Record<string,unknown>)=>Promise<void>;deleteScreen:(id:string)=>Promise<void>;listDrawings:(ticker:string)=>Promise<Drawing[]>;refresh:()=>Promise<void>}
const OwnerContext=createContext<Context|null>(null)

export function OwnerProvider({children}:{children:ReactNode}){
  const client=ownerClient,[user,setUser]=useState<User|null>(null),[googleEnabled,setGoogleEnabled]=useState<boolean|null>(null),[loading,setLoading]=useState(true),[error,setError]=useState(''),[watchlist,setWatchlist]=useState<string[]>([]),[screens,setScreens]=useState<SavedScreen[]>([]),[alerts,setAlerts]=useState<OwnerAlert[]>([]),[states,setStates]=useState<AlertState[]>([]),userRef=useRef<User|null>(null)
  ...
  useEffect(()=>{let live=true;fetch(`${URL.replace(/\/$/,'')}/auth/v1/settings`,{headers:{apikey:KEY}}).then(response=>response.ok?response.json():null).then(settings=>{if(live)setGoogleEnabled(typeof settings?.external?.google==='boolean'?settings.external.google:null)}).catch(()=>{if(live)setGoogleEnabled(null)});return()=>{live=false}},[])
  const signIn=async()=>{setError('');if(googleEnabled!==true){const next=new Error('Google sign-in is not enabled yet. Use the magic-link fallback.');setError(next.message);throw next}const redirectTo=`${location.origin}${import.meta.env.BASE_URL}${location.search}`;const{error:next}=await client.auth.signInWithOAuth({provider:'google',options:{redirectTo,queryParams:{prompt:'select_account'}}});if(next){setError(next.message);throw next}}
  const sendMagicLink=async(email:string)=>{setError('');setLoading(true);const{error:next}=await client.auth.signInWithOtp({email:email.trim(),options:{shouldCreateUser:false,emailRedirectTo:`${location.origin}${import.meta.env.BASE_URL}`}});setLoading(false);if(next){setError(next.message);return false}return true}
  ...
  const value=useMemo<Context>(()=>({user,googleEnabled,loading,error,watchlist,screens,alerts,states,signIn,sendMagicLink,signOut,toggleWatch,saveScreen,deleteScreen,listDrawings,refresh}),[user,googleEnabled,loading,error,watchlist,screens,alerts,states,refresh])
